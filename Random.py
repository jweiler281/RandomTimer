...
def csvWriter(fileN,text):
        ...
        with open(fileN, 'a') as csvfile: # Appends lines using 'a' flags
...
def manyIterations(iterNum=10000):
        # Used for testing purposes only
        # Generates test run data for statistical analysis
        # iterNum is the number of results to be gained through testing
        if os.path.exists('teststat.csv') == True: # see if teststat.csv exists
                os.remove('teststat.csv') # if so, delete it
